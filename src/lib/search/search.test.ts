...
describe("search ranking", () => {
  ...
  it("ranks in-neighborhood 2-bedrooms first for the flagship query", () => {
    const { results } = search(flagship, ctx);
  ...
  it("explains the trade-off in the anchor listing", () => {
    const r = search(flagship, ctx).results.find((x) => x.listing.id === "dv-0901")!;
  ...
  it("puts listings missing a must-have below those that have it", () => {
    const { results } = search(intent({ neighborhoods: ["احمدآباد"], mustHave: ["parking"], minRooms: 2 }), ctx);
  ...
  it("states the sample size of the neighborhood median", () => {
    const r = search(intent({ maxDeposit: 500 * M, neighborhoods: ["وکیل‌آباد"], minRooms: 2 }), ctx).results.find(
  ...
  it("scores stay within 0–100", () => {
    for (const r of search(intent({}), ctx).results) {
...
describe("suggest", async () => {
  ...
  it("relaxes a too-low rent budget", () => {
    const s = suggest({ ...EMPTY_INTENT, maxRent: 5 * M, minRooms: 0, maxRooms: 0 }, ctx);
